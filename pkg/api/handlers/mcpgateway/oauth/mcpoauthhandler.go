...
func (f *MCPOAuthHandlerFactory) CheckForMCPAuth(req api.Context, mcpServer v1.MCPServer, mcpServerConfig mcp.ServerConfig, userID, mcpID, oauthAppAuthRequestID string) (string, error) {
	switch mcpServerConfig.Runtime {
	...
	go func() {
		defer close(errChan)
		_, err := f.mcpSessionManager.ClientForMCPServerWithOptions(req.Context(), "Obot OAuth Check", mcpServer, mcpServerConfig, nmcp.ClientOption{
