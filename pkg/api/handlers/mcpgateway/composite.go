...
func (h *Handler) onCompositeMessage(ctx context.Context, msg nmcp.Message, m messageContext) {
	...
	for _, componentServer := range m.componentServers {
		var (
			componentKey = normalizeName(componentServer.mcpServer.Name)
			scope        = fmt.Sprintf("%s.%s", msg.Session.ID(), componentServer.mcpID)
		)
	...
	case methodInitialize:
		go func(session *nmcp.Session) {
			...
			for _, client := range clients {
				if err := h.mcpSessionManager.CloseClient(ctx, client.serverConfig, client.scope); err != nil {
	...
	case methodToolsList:
		...
		for componentKey, client := range clients {
			...
			for _, tool := range result.Tools {
				...
				compositeTool.Name = fmt.Sprintf("%s_%s", componentKey, compositeTool.Name)
	...
	case methodToolsCall:
		...
		// TODO: Reverse overrides on CallToolRequest
...
func (c *compositeContext) toCompositeTool(componentKey string, tool nmcp.Tool) (*nmcp.Tool, error) {
	...
	if overrideName := override.OverrideName; overrideName != "" && tool.Name != overrideName {
		tool.Name = fmt.Sprintf("%s_%s", componentKey, overrideName)
	}
...
func (c *compositeContext) toCompositeToolParameters(toolKey string, inputSchema json.RawMessage) (json.RawMessage, error) {
	...
	for propertyName, property := range schema.Properties {
		...
		if override.OverrideDescription != "" {
			property.Description = override.OverrideDescription
		}
		...
		if overrideName := override.OverrideName; overrideName != "" && overrideName != propertyName {
			...
			schema.Properties[overrideName] = property

			// Update required array if the parameter was required
			for i, reqName := range schema.Required {
				if reqName == propertyName {
					schema.Required[i] = overrideName
					break
				}
			}
		} else {
