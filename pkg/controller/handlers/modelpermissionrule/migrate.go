...
// MigrateAgentAllowedModels ensures that an agent's AllowedModels and AllowedModelProvider fields are cleared.
// This enables owners and admins to control model access for threads with ModelPermissionRules.
// For the default agent, this handler preserves the existing AllowedModels as a "user wildcard" MPR,
...
// threads descended from the agent to use models granted via ModelPermissionRules.
func MigrateAgentAllowedModels(req router.Request, _ router.Response) error {
	agent := req.Object.(*v1.Agent)

	if len(agent.Spec.Manifest.AllowedModels) == 0 && len(agent.Spec.Manifest.AllowedModelProviders) == 0 {
		// Migration complete, bail out
		return nil
	}

	// Preserve any existing chat configuration as an MPR before clearing it from the agent's manifest.
	if agent.Spec.Manifest.Default {
		var (
			...
			ruleSubjects = []types.Subject{
				{
					Type: types.SubjectTypeSelector,
					ID:   "*",
				},
			}
			ruleModels = convertToModelResources(agent.Spec.Manifest.AllowedModels)
		)

		// Make sure there are actually rules to copy before attempting to avoid producing an invalid MPR.
		if len(ruleModels) > 0 {
			if err := req.Get(&rule, agent.Namespace, ruleName); err == nil {
				// Rule already exists, determine if it's drifted from the initial configuration before the migration could finish
				if !utils.SlicesEqualIgnoreOrder(ruleModels, rule.Spec.Manifest.Models) ||
					!utils.SlicesEqualIgnoreOrder(ruleSubjects, rule.Spec.Manifest.Subjects) {
					// Replace the rule's subjects and models
					rule.Spec.Manifest.Subjects = ruleSubjects
					rule.Spec.Manifest.Models = ruleModels
				...
				// Create a new rule
				if err := req.Client.Create(req.Ctx, &v1.ModelPermissionRule{
	...
	// Clear AllowedModels and AllowedModelProviders from the agent.
	// This does two things:
	// 1. Prevent this migration handler from running again
	// 2. Allows threads descended from the agent to run invoke using models granted to the user via MPRs
	agent.Spec.Manifest.AllowedModels = nil
	agent.Spec.Manifest.AllowedModelProviders = nil

	return req.Client.Update(req.Ctx, agent)
}

// MigrateAgentDefaultModel ensures that an agent's default model field is cleared.
// When migrating the default agent, this handler will attempt to update the "LLM" DefaultModelAlias to point
