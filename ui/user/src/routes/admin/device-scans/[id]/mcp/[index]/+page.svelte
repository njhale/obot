<script lang="ts">
	...
	let parentPlugin = $derived(findParentPlugin(scan, server?.plugin_file));
			...
			<div class="dark:bg-surface2 bg-background flex flex-col gap-3 rounded-md p-4 shadow-sm">
				...
				<dl class="grid grid-cols-1 gap-x-6 gap-y-2 text-sm md:grid-cols-[max-content_1fr]">
					...
					{#if server.config_file}
						<dt class="text-on-surface1">Config file</dt>
						<dd class="font-mono text-xs break-all">{server.config_file}</dd>
					{/if}
					{#if parentPlugin}
					...
					{:else if server.plugin_file}
						<dt class="text-on-surface1">Plugin file</dt>
						<dd class="font-mono text-xs break-all">{server.plugin_file}</dd>
					{/if}
