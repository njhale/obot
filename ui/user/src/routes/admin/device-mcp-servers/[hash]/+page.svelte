<script lang="ts">
	...
	import {
		AdminService,
		type AggregatedDeviceMCPServerDetail,
	...
	let detail = $derived<AggregatedDeviceMCPServerDetail | null | undefined>(data?.detail);
	...
	let lastSeen = $derived(detail ? formatTimeAgo(detail.last_seen).relativeTime : '');
...
<Layout
	title="MCP Server"
	showBackButton
	onBackButtonClick={() => goto('/admin/device-mcp-servers')}
>
	<div
			...
			<div class="flex flex-col gap-2">
				...
				<Table
					...
					onClickRow={(d, isCtrlClick) => {
						openUrl(`/admin/device-scans/${d.device_scan_id}/mcp/${d.index}`, isCtrlClick);
					}}
				>
