<script lang="ts">
	...
	// CONFIG QUEUE (derived by joining projectMCP.mcpID to user single/remote servers)
	...
	onMount(() => {
		const handleVisibilityChange = () => {
			...
			if (document.visibilityState === 'visible') {
				// Fire-and-forget refreshes; order is OAuth first, then servers
				checkOauths();
				refreshUserServers();
				// Opening handled by reactive effects after data refresh
			}
		};
	...
	async function prepareConfigureForm(server: MCPCatalogServer, parent?: MCPCatalogEntry) {
		...
		try {
			...
			// ignore 404s and unexpected errors same as existing flow
			values = {};
		}
