...
<div class="flex w-full flex-col gap-4 p-5" in:fade>
	...
	{#if !template}
		<div class="flex flex-col items-center gap-3 py-8">
			<p class="text-sm text-gray-600 dark:text-gray-300">
				Create a snapshot of the project to share with other users.
			</p>
			<button class="button-primary" onclick={createFromSnapshot}>Create snapshot</button>
		</div>
	{:else}
				...
				<div class="flex flex-col divide-y divide-gray-100 dark:divide-gray-700">
					{#if template.created}
						<div class="p-3">
							...
							<p class="text-sm text-gray-600 dark:text-gray-300">{formatDate(template.created)}</p>
						</div>
					{/if}

					{#if template.projectSnapshotRevision}
						<div class="p-3">
							<h4 class="mb-1 text-xs font-medium text-gray-500">Snapshot Revision</h4>
							<p class="text-sm text-gray-600 dark:text-gray-300">
	...
	{#if template}
		<Confirm
			msg={`Are you sure you want to delete this snapshot: ${template.projectSnapshot.name || 'Unnamed Project Snapshot'}?`}
			show={toDelete}
