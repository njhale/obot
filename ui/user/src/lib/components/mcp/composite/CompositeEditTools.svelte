<script lang="ts">
	...
	let prefixInvalid = $derived(!isValidToolPrefix(toolPrefix ?? ''));
	...
	let hasBlockingToolNameErrors = $derived(
		tools.some((t) => {
			...
			if (toolNameIssues(name).some((i) => i.severity === 'error')) return true;
...
>
	...
	<div class="relative flex flex-col gap-2 overflow-x-hidden p-4">
		<div class="flex flex-col gap-1">
			<p class="text-xs text-gray-500">Tool name prefix</p>
	...
	<div class="bg-surface1 sticky bottom-0 left-0 mt-4 flex w-full justify-end gap-2 p-4">
		...
		<button
			...
			disabled={hasBlockingToolNameErrors || prefixInvalid}
			onclick={() => {
				onSuccess?.();
				dialog?.close();
			}}>Confirm</button
