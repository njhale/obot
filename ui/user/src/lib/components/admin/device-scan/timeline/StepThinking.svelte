<script lang="ts">
	...
	let durationMs = $derived(step.durationMs ?? 0);
</script>
...
<div class="border-surface2 dark:border-surface3 flex flex-col gap-2 rounded-md border p-2">
	...
	>
		...
		{/if}
		<Brain class="mt-0.5 size-3.5 shrink-0 opacity-70" />
		<span class="text-on-surface1 shrink-0 text-xs font-medium tracking-wide uppercase">
			Thinking
		</span>
		<span class="text-on-surface1 min-w-0 flex-1 truncate font-mono text-xs">
			{preview || '(empty)'}
		</span>
	...
	{#if open}
		<div class="flex flex-col gap-2 pl-6">
			...
			<dl class="text-on-surface1 grid grid-cols-[max-content_1fr] gap-x-3 gap-y-0.5 text-[10px]">
				...
				{#if durationMs > 0}
					<dt>Duration</dt>
					<dd class="font-mono">{formatNumber(durationMs)} ms</dd>
				{/if}
			</dl>
		</div>
	{/if}
